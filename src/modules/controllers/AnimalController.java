...
public class AnimalController {
    ...
    private static final String INSERT_SQL = """
        ...
        (name, sex, species, breed, size, color, birthdate, microchip, rga, fiv, felv, status, notes, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())
        """;
    ...
    private static final String UPDATE_SQL = """
        ...
        SET name = ?, sex = ?, species = ?, breed = ?, size = ?, color = ?, birthdate = ?, microchip = ?, rga = ?, fiv = ?, felv = ?, status = ?, notes = ?, updated_at = now()
        WHERE id = ?
        """;
    ...
    public static void addAnimal(Connection conn, Animal a) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(INSERT_SQL)) {
            ...
            ps.setString(10, a.getFiv());
            ps.setString(11, a.getFelv());
            ps.setString(12, a.getStatus());
            ps.setString(13, a.getNotes());
            ps.executeUpdate();
        }
    }
